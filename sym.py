...
root = Tk()
root.iconbitmap(default = 'C:\image\python_imagesearch\symbol.ico')
root.title("심알바")
...
def run__sym(cool):
    while True:
        pos = imagesearch("C:\image\python_imagesearch\sym.png")
        if pos[0] != -1:
            print("홀심 매칭 성공")
            ...
            notification.notify(
                title='심알바',
                message='심이 끝났습니다',
                app_icon='C:\image\python_imagesearch\symbol.ico',
                app_name="SymAlba",
                timeout=5,
            )
        else:
